import { ArrowLeft, Share2, Calendar, Clock, ShieldCheck, MessageCircle, Check } from "lucide-react"
...
<div className="flex items-center gap-3 mb-5 my-4">
  ...
  >
    {post.author}
          ...
          {/* 상태 뱃지 (대여가능 62x30, 과잠 43x30, radius 40, 글씨 12px regular) */}
          <div className="flex gap-2 mb-4">
            ...
            <span className="w-[43px] h-[30px] flex items-center justify-center text-[12px] font-normal bg-[#E9E8FF] text-[#1A1A1A] rounded-[40px]">
              {post.category}
            </span>
          </div>
...
>
  {post.title}
          ...
          {/* 설명 (width 305, height 34, font14 regular) */}
<p 
...
>
  {post.description}
          ...
          {/* 대여 신청일 (아이콘 18x18, 텍스트 width166 font12) */}
<div className="flex items-center gap-2 mb-2">
  ...
  >
    대여 신청일 : {post.applyDate}
  </span>
</div>
          {/* 날짜 시간 */}
          <div className="flex items-center gap-2 mb-9">
            <Clock size={22} className="text-[#7F7F7F] flex-shrink-0" />
            <span className="text-[12px] text-[#7F7F7F]">{post.dateTime}</span>
          </div>
